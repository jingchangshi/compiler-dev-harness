 ...
 * `auto` (capability-based A/B experiment: Ripwire `--pack-task --json` when
 * usable, controlled legacy fallback otherwise), `ripwire` (explicit
 * experiment; degrades loudly, never silently becomes legacy), or `legacy`
 * (the retained rg/git retrieval — the REPOSITORY DEFAULT while Ripwire is
 * experimental, per the R1 KEEP_RIPWIRE_EXPERIMENTAL decision). Every result
 * discloses `backend`/`fallback`/`fallback_reason` (finite reason vocabulary;
 * no silent fallback), git state/diff/history and MLIR log forensics stay
 * provider-independent, and one non-sensitive context observation line is
 * emitted per source-retrieval attempt. Ripwire output is normalized into a
 * bounded `source_context` object whose disclosures keep
 * weak/ambiguous/truncation visible, and the rendered bundle labels Ripwire
 * evidence as generic retrieval/ranking context — never as an mlir-repomap
 * semantic fact.
 *
 * v1.4 driver, Phase R1.5: the repository default moved behind an explicit
 * constant; v1.5 driver, Phase R1.6: observation protocol v2 fixed provider
 * attribution (attempts vs served vs delivery state) — Gate A; v1.6 driver,
 * Phase R1.7 (Gate B): the repository default became `auto`, so normal usage
 * attempts Ripwire first with controlled legacy fallback. Explicit `ripwire`
 * remains strict; explicit `legacy` remains the control backend.
 *
...
/**
 * One analyzer-consumable line: the SERVED backend (or `none` when the call
 * degraded and nothing served), the fallback fact, the delivery state, and the
 * disclosure block. Format note for the offline analyzer: the `fallback:`
 * parenthetical stays stable; `delivery=degraded: <reason>` marks an
 * explicit-Ripwire failure where no provider served at all.
 */
function contextBackendLine(bundle) {
  ...
  const degradedReason = bundle.delivery_state === 'degraded' ? bundle.source_context?.error ?? 'unknown' : null
  const deliveryPart = degradedReason !== null ? `degraded: ${degradedReason}` : String(bundle.delivery_state ?? 'served')
  const disclosures = d !== null && d !== undefined
    ...
    : ''
  return `Context backend: ${bundle.backend} (fallback: ${fallbackPart}) | delivery=${deliveryPart}${disclosures}`
}
...
exports.apply = function apply(ctx) {
  ...
  const driverPromise = import(new URL('./compiler-inspect-driver.mjs?v=1.5', `file://${__filename}`).href)
