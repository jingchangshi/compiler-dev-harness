...
exports.apply = function apply(ctx) {
  ...
  ctx.tools.register({
    ...
    description: 'Deterministic scaffolding, validation, readiness, and staleness for code-explanation teaching artifacts (Teaching Artifact Protocol v1). Commands: plan — scaffold a teaching bundle for a subject (subject skeleton, evidence plan over the existing deterministic tools, extension fields, readiness checklist, audience questions); validate — validate subject/evidence/dossier/handoff shapes and evidence discipline in a bundle dir; readiness — run the mechanical readiness gate and, when semantic_review is provided, record it and persist readiness.json (READY requires mechanical pass AND a recorded audience-comprehension review); stale — compare the bundle\'s recorded provenance (HEAD, source file hashes) against the repository now. The tool never generates teaching content: mechanisms, mental models, storylines, and the semantic verdict are agent reasoning recorded in the artifacts.',
    parameters: { type: 'object', additionalProperties: false, required: ['command'], properties: {
      command: { type: 'string', enum: ['plan', 'validate', 'readiness', 'stale'], description: 'plan (scaffold), validate (schema + evidence discipline), readiness (mechanical gate + semantic review recording), stale (provenance freshness).' },
      ...
      bundle_dir: { type: 'string', description: 'validate/readiness/stale: teaching bundle directory (under analysis/explanations/ by default).' },
