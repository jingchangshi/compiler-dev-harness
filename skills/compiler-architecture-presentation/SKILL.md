...
5. **Scaffold + write QMD** (see Quarto project workflow reference). Titles short, Chinese-first, no generator provenance. Content forms are canonical, not free-style — follow `references/quarto-content-semantics.md`:
   - slide-level evidence goes in a `::: footer` div as the last block of the slide; `.footnote[...]` is forbidden (it renders literally in Quarto Reveal.js) and fails the checker;
   - parallel items are real `- ` lists — a paragraph joining ≥ 3 items with `·` separators fails the checker;
   - `## Header text` lines are the slide identity the manifest records, so keep them short, unique, and stable.
...
```
   ...
   The checker is deterministic and covers: project structure, manifest coverage, Quarto content semantics (footer/list forms above), and diagram geometry — every `diagrams/*.excalidraw` is validated for node overlap, edges crossing nodes, extreme aspect ratio, projected text size at 1600×900, and label overflow. If geometry or a semantic check fails, fix the `.excalidraw`/QMD (or re-run `spec_to_diagram.py`) — never weaken the checker.
...
## Semantic visual contract (Phase T6)

Mechanism visuals must not just *look* plausible — they must consume the semantics the producer declared. The TeachingDossier (producer) owns:

- `mechanism.stages[]` — source-derived stage names;
- `mechanism.states[]` — state entities with lifecycles (`kind` ∈ work_queue | live_sequence | analysis | derived | ir | accounting | other; `created_in` / `read_in` / `updated_in` / `finalized_in` stage references) — including which states are **initialization-only derived** (`updated_in: []`);
- `mechanism.control_relations[]` — typed stage→stage control relations (`kind` ∈ next | success | failure | reject | skip | requeue | retry | loop | finalize).

The handoff visual spec (still semantic-only — no geometry) may then declare, per visual:

- `covers: ["mechanism" | "control_flow" | "state_lifecycle"]` — what the visual claims to answer; obligations bind to claims;
- `nodes[].mechanism_stages` / `nodes[].stage_merge_reason` — which stages a node depicts (aggregating >1 stage requires an explicit reason — never a silent fold);
- `nodes[].state_refs` / `nodes[].state_merge_reason` — which declared states a node depicts (mixing different state families requires a reason);
- `edges[].domain` (control | state | data) + `edges[].kind` (per-domain enum) — a reject path is a typed control edge, not an unlabeled arrow; state access edges name `create` / `read` / `update` / `finalize`;
- `stage_dispositions` / `deferred_relations` — explicit, reasoned non-mappings (deferred needs a reason and, optionally, a `to_visual` pointer).

Deterministic enforcement (`node <harness>/scripts/check-visual-semantics.mjs <bundle-dir>`, also folded into readiness + preflight, so an unfaithful bundle is not CONSUMABLE):

- **stage coverage** — a `mechanism`-claiming visual maps or dispositions every declared stage (silent stage loss = error);
- **branch coverage** — a `control_flow`/`mechanism` claim with `has_important_branching=true` must show a reject-family path or defer one with a reason;
- **lifecycle contradiction** — an update claim against an initialization-only state (`updated_in: []`) is an error; state access is also checked against the declared stage (`updated_in`/`created_in`/`read_in`/`finalized_in`);
- **state-role collapse** — one node silently depicting states of different families (e.g. a scheduling queue vs the live sequence) is an error;
- **control-relation coverage** — every declared relation must be consumed by some visual edge or explicitly deferred with a reason;
- **SPLIT_RECOMMENDED** — bounded complexity heuristic (≥5 stages, ≥2 mutable state families, loop edge, reject-family edge) recommending 2 related visuals over one compressed figure; always a recommendation, never an error.

Presentation decisions stay here: state-only nodes render amber, state-access edges render dashed, stage-mapped nodes render blue (see `spec_to_diagram.py`); the checker never judges aesthetics, and geometry QA (`check_diagram_geometry.py`) still owns physical validity.
...
6. **Excalidraw-first diagrams.** Editable `.excalidraw` source first, paired SVG referenced from QMD; keep labels short and reserve CJK width; node boxes must not overlap; edges avoid crossing nodes; REJECT branches stay separated; long pipelines get enough horizontal spacing; **split overloaded figures instead of shrinking text — and split semantically overloaded figures instead of compressing distinct mechanism stages, state families, or control paths into fewer nodes (Phase T6: split > semantic compression)**. These are not aspirations: `check_project.py` enforces geometry deterministically per `diagrams/*.excalidraw` (Phase T5), the semantic visual contract is enforced on the bundle by `check-visual-semantics.mjs` (Phase T6), and `spec_to_diagram.py` emits measured, wrapped, routed layouts that satisfy the geometry gates by construction.
7. **Canonical QMD forms.** Evidence footers (`::: footer`), semantic lists (one `- ` item per parallel element), columns, speaker notes, and diagram references follow `references/quarto-content-semantics.md`; deviations that render literally (`.footnote[...]`) or collapse lists into prose fail the checker.
