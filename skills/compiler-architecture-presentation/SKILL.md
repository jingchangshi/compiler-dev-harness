...
6. **Excalidraw-first diagrams.** Editable `.excalidraw` source first, paired SVG referenced from QMD; keep labels short and reserve CJK width; node boxes must not overlap; edges avoid crossing nodes; REJECT branches stay separated; long pipelines get enough horizontal spacing; split overloaded figures instead of shrinking text. These are not aspirations: `check_project.py` enforces them deterministically per `diagrams/*.excalidraw` (Phase T5), and `spec_to_diagram.py` emits measured, wrapped, routed layouts that satisfy them by construction.
