...
- `canonical_example` — for pass/algorithm/pipeline/subsystem/workflow/component_group it is mechanically required at standard+ depth, and at presentation depth it must be a **worked** example: ≥3 ordered trace steps (`execution_trace`/`steps`), each optionally tied to a `mechanism.stages` name. Provenance kinds: `test` > `production` > `probe` > `reconstructed`; label reconstructed examples and never present them as executed.
- `worked_examples` (dossier) — per-mechanism step-by-step instances for the complex mechanisms the audience must trace; the handoff declares which the deck must show (`handoff.worked_examples[{id,title,summary}]`), and the deck checker fails if any is silently dropped.
