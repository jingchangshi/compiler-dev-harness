...
| `mechanism` | `{summary?, stages[], control_flow?, data_flow?, mutable_state?, has_important_branching?, states?, control_relations?}` | stages `{name, what, where?, key_functions?, evidence_refs}` — names derived from source. `states[]` (Phase T6): `{id, name, kind, created_in?, read_in?, updated_in?, finalized_in?, evidence_refs, note?}` with `kind` ∈ `work_queue\|live_sequence\|analysis\|derived\|ir\|accounting\|other` — the declared lifecycle the presentation must not contradict; `updated_in: []` marks an initialization-time derived state. `control_relations[]` (Phase T6): `{from, to, kind, condition?, evidence_refs, note?}` with `kind` ∈ `next\|success\|failure\|reject\|skip\|requeue\|retry\|loop\|finalize`; self-relations are allowed (loop backedge). Stage references in both must resolve against `stages[].name` — the semantic checker enforces it |
...
- Semantic visual contract (Phase T6, all optional; legacy specs without them stay valid): a visual may declare `covers` (`mechanism|control_flow|state_lifecycle`) — obligations bind to these claims. Nodes carry `mechanism_stages` / `state_refs` (arrays of declared stage names / state ids; aggregating >1 requires `stage_merge_reason` / `state_merge_reason`). Edges carry `domain` (`control|state|data`, default control) + `kind` (control: `next|success|failure|reject|skip|requeue|retry|loop|finalize`; state: `create|read|update|finalize`; data: `flow|dependency`) and optional `states`. Visuals may carry `stage_dispositions` (`{stage, disposition: deferred|merged, reason, to_visual?}`) and `deferred_relations` (`{from, to, kind, reason}`) for explicit, reasoned non-mappings. Enforcement lives in `scripts/check-visual-semantics.mjs` and the readiness `visual_semantics` check: silent stage loss, lifecycle contradictions (e.g. an update claim on a state declared `updated_in: []`), state-role collapse, and unconsumed declared relations are deterministic errors; `SPLIT_RECOMMENDED` (many stages + state families + loop + branch on one visual) is a recommendation to split the figure, never an error.
