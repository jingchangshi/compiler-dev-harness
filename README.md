...
`compiler_inspect` gained a code-context provider seam with an explicit backend policy — `backend: auto | ripwire | legacy` (input, or `COMPILER_INSPECT_BACKEND` for A/B without source edits). The **repository default is `legacy`** while Ripwire is experimental (R1 decision: KEEP_RIPWIRE_EXPERIMENTAL) — installing the binary alone never flips production traffic; promotion requires changing `REPOSITORY_DEFAULT_BACKEND_POLICY` in `compiler-context-backend.mjs` in one deliberate commit. `ripwire` is the explicit experiment and degrades loudly instead of silently switching; `auto` is the capability-based A/B experiment (Ripwire when usable, controlled legacy fallback otherwise). No percentage rollout, no random routing. Only the generic source retrieval swaps: git state/diff/history and the MLIR log forensics run unchanged in every mode, and Ripwire remains an implementation detail behind the same three model-facing tools.
