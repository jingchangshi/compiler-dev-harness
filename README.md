...
`compiler_inspect` gained a code-context provider seam with an explicit backend policy — `backend: auto | ripwire | legacy` (input, or `COMPILER_INSPECT_BACKEND` for A/B without source edits). `auto` (default) uses [Ripwire](https://github.com/redhat-et/ripwire) `--pack-task --json` as the primary generic source-context provider and falls back to the retained rg/git retrieval on any failure; `ripwire` is explicit and degrades loudly instead of silently switching; `legacy` forces the rg path for regression and A/B. Only the generic source retrieval swaps: git state/diff/history and the MLIR log forensics run unchanged in every mode, and Ripwire remains an implementation detail behind the same three model-facing tools.
