...
## Explanation orchestration control plane (Phase T4)

T1–T3 made the capability pieces deterministic; T4 makes the *request* deterministic. A new orchestration layer (`compiler-orchestrate-driver.mjs`) sits above the knowledge plane and turns a natural-language request — subject names plus desired outputs — into a resumable execution plan, so the user never needs to know bundle paths, compose commands, or artifact internals:

- **Artifact catalog** (`catalog`) — derived on every call from the source-of-truth artifacts across two stores: the gitignored **runtime store** (`analysis/runtime/explanations/<repository>/`, `analysis/runtime/presentations/`, `analysis/runtime/runs/` — normal use never dirties the tracked harness tree) and the **curated fixtures** (`analysis/explanations/`, `analysis/presentations/` — dogfood/regression evidence, promoted only by explicit developer action). Readiness and freshness are recomputed, never trusted from stored verdicts; there is no catalog database.
- **Subject resolution** (`run-plan`) — user names resolve through deterministic keys only (subject id, exact/normalized/condensed name, type canonical ids such as a pass argument). No deterministic match → CREATE; several distinct identities → AMBIGUOUS with all candidates returned for the agent to decide (never first-match-wins).
- **Lifecycle planning** — per subject, deterministic REUSE (READY + FRESH + compatible depth: presentation > deep > standard > overview + same repository/HEAD) / REFRESH (stale, HEAD drift, or too shallow) / CREATE, plus composition reuse (identical requested component set + recursively fresh) and presentation reuse (existing deck manifest matches the current handoff hash). The plan is an explicit execution DAG with dependency-ordered nodes.
- **Run state + resume** — an `explanation_run` (`analysis/runtime/runs/<repository>/<run-id>.json`) records coordination facts only; node truth is re-derived from the artifacts on every `run-status`. An open run with the same request signature is resumed, never duplicated (idempotent re-planning); interrupted runs survive sessions.
- **Child work packets + parent validation** — CREATE/REFRESH subjects get compact, self-contained packets (subject, type, repository, HEAD, target runtime bundle root, required depth, why, required outputs); independent packets are marked parallelizable for subagent delegation. A child's "done" advances nothing: `run-status` re-validates readiness + freshness, renders the derived documents (`explanation.md` per subject via `render`, `system-story.md` for the system) into the runtime store, and emits next actions. Derived documents for curated-origin bundles render into the runtime store, keeping the tracked tree clean.
- **Final gate** (`run-finalize`) — COMPLETE only when every requested deliverable verifies: all subject bundles READY+FRESH at compatible depth, all requested documents current with their JSON, the composition recursively fresh with no open conflicts (`blocked_at_composition` otherwise), and the presentation manifest matching the current handoff hash (`presentation_invalid` otherwise).

The semantic work is unchanged and stays with the agent: mechanisms, mental models, bridges, storylines, semantic reviews. Orchestration adds no knowledge layer — it only decides what is already known, what is missing, what to do next, and whether the request is actually done. Always-on routing lives in the `code-explanation-orchestration-policy` prompt section and `skills/code-explanation/`. Detailed contract: ARCHITECTURE §20.
