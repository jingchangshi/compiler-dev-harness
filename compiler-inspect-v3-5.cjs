 ...
 * semantic fact.
 *
 * v1.4 driver, Phase R1.5 (Workstream A): the repository default backend
 * policy became `legacy` — installing the binary alone no longer flips
 * production traffic; promotion requires changing
 * REPOSITORY_DEFAULT_BACKEND_POLICY in compiler-context-backend.mjs in one
 * deliberate, reviewed commit. No percentage rollout, no random routing.
 *
...
exports.apply = function apply(ctx) {
  ...
  const driverPromise = import(new URL('./compiler-inspect-driver.mjs?v=1.4', `file://${__filename}`).href)
